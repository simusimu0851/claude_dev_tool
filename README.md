...
- **개발 파이프라인** — 계획 → 실행 → [리뷰 ↔ 수정 루프] (설계 브리핑 기반, advisor 도구로 Opus 선택적 호출)
...
### 6. 개발 파이프라인 (계획 → 실행 → [리뷰 ↔ 수정 루프])
코드 요청 감지 시 자동 실행:
1. **계획**: 수정/생성 파일, 구현 순서, 리스크 분석 (코드 작성 없음)
2. **실행**: 계획 기반 코드 작성 + diff 자동 적용
3. **리뷰 ↔ 수정 루프** (최대 3회):
   - **리뷰**는 단순 PASS/FAIL이 아닌 **설계 브리핑** 형식으로 출력:
     - `## 사용자 의도와의 정합성` (충족/누락)
     - `## 개선 필요 항목` (P1, P2… 우선순위 순)
     - `## 보완할 점` (엣지 케이스·리스크)
     - `## 결론` (`REVIEW_PASS` / `REVIEW_FAIL`)
   - **수정**은 이 브리핑을 **설계도**로 삼아 Haiku 중심으로 처리 (명확한 항목만 Haiku, 정말 애매할 때만 advisor)
   - 수정분 반영 → 다음 루프에서 재리뷰
   - **종료 조건**: `REVIEW_PASS` / 사용자 `n·s` 입력 / 최대 루프 도달

각 단계 특징:
- **단계 전 확인**: `Enter/y=진행, n=취소, s=스킵` — 리뷰/수정 루프도 매 반복마다 다시 물음
- **advisor 도구(Opus) 활성화**: Haiku가 복잡한 추론이 필요할 때만 Opus에 위임. 수정은 브리핑이 설계도이므로 advisor 호출률이 낮음 → 고비용 모델 사용 최소화
- **브리핑**: 각 단계에서 무슨 일이 일어나는지 + 사용된 advisor 여부 + 토큰 + 루프 인덱스 출력
- **prefix caching**: 단계/루프 간 동일 prefix 자동 캐시 재사용
- **루프 비용 상한**: `_MAX_REVIEW_LOOPS=3` 상수로 폭주 방지
- **단순 질문은 스킵**: `설명/왜/what` 등 질문은 파이프라인 없이 직접 응답
...
# 4단계: 개발 요청 (코드 키워드 감지 시 파이프라인 시작)
로그인 에러 핸들링을 개선해줘
→ [1/4] 계획 수립할까요? [y/n/s] > y
   📋 계획 출력 + advisor 토큰 표시
→ [2/4] 실행할까요? [y/n/s] > y
   🤖 구현 + diff 자동 적용
→ [3/4 리뷰 (설계 브리핑)] 리뷰할까요? [y/n/s] > y
   🔍 # 리뷰 브리핑 / ## 개선 필요 항목 / ## 결론 REVIEW_FAIL
→ [4/4 수정 (브리핑 기반)] 수정할까요? [y/n/s] > y
   🔧 브리핑 항목 순으로 diff 적용 (Haiku 중심)
→ [리뷰 루프 2/3] 리뷰할까요? [y/n/s] > y
   🔍 ## 결론 REVIEW_PASS  → 루프 종료 (또는 n 입력으로 중단)
...
`output_path`는 생략하면 `project_path`와 동일 — 작업 폴더가 곧 출력 폴더입니다.
`PROJECT.md`·`SESSION_STATE.md`·생성된 파일·`.bak` 백업이 모두 한 곳에 모입니다.
메타 파일을 별도 폴더로 빼고 싶을 때만 `output_path`를 지정하세요.

### CLI 인자

```bash
# 기본: 작업 폴더 = 출력 폴더
python dev_pipeline_v4.py --engine unity --path /my/game/project

# 출력 폴더를 분리하고 싶을 때만
python dev_pipeline_v4.py --path /my/game/project --save-path ./output
```
...
| **pipeline_controller.py** | 요청 오케스트레이션 (리뷰-수정 루프) + 상태 생성 |
...
├── pipeline_controller.py     # 요청 오케스트레이션 (계획→실행→리뷰↔수정 루프)
...
> `--save-path`를 지정하면 `PROJECT.md` 등 메타 파일은 그 폴더로 분리되고,
> `.bak` 백업과 diff 패치 대상은 작업 폴더(`--path`)에 남습니다.
